...
const registerValidationSchema = Joi.object({
  ...
  role: Joi.string().valid('owner', 'renter').required().messages({
    ...
    'any.only': 'role field must be one of "owner", "renter"'
  }),
...
const loginApiValidator = async (req, res, next) => {
  const {
    error
  } = await loginValidatonSchema.validate(req.body);
  if (error) {
    next(error);
  }
  next();
};
const registerApiValidator = async (req, res, next) => {
  const {
    error
